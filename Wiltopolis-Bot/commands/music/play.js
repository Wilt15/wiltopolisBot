...
try {
		...
		try {
			...
			try {
				...
				let results = new Discord.MessageEmbed()
					...
					.setColor("#00A2E8")
					.setDescription(videos.map(video2 => `**${++index}.** ${video2.title.replace(/&#39;/g, "'").replace(/&quot;/g, "\"").replace(/&amp;/g, "&")}`).join('\n'))
					.setFooter("Please provide a value to select one of the search results ranging from 1-10.");
				message.channel.send(results).then(m => m.delete({
					timeout: 10000
				}));
	...
	async function play(guild, song) {
		...
		const dispatcher = serverQueue.connection.play(await ytdl(song.url, { highWaterMark: 1<<25 }))
			.on('end', reason => {
