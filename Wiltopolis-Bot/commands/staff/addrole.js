...
module.exports.run = async (bot, message, args) => {
  ...
  if (!role) {
    ...
    if (sorted[0].distance > 0) {
      ...
      coll.on("collect", m => {
        if (m.content.toLowerCase() === "yes") {
          if (member.roles.has(message.guild.roles.find(x => x.name === sorted[0].role).id)) return errors.custom(message, 'That user already has that role.')
          member.roles.add(message.guild.roles.find(x => x.name === sorted[0].role));

          const yesSuccess = new Discord.MessageEmbed()
            .setAuthor("Role Added", member.user.avatarURL())
            .setDescription(`**Added By**: ${message.author}\n**Member**: ${member.user}\n**Role**: ${message.guild.roles.find(x => x.name === sorted[0].role)}`)
          ...
          coll2.on("collect", msg => {
            ...
            if (member.roles.has(message.guild.roles.find(r => r.name === sorted[parseInt(msg.content)].role).id)) return errors.custom(message, 'That user already has that role.')
            member.roles.add(message.guild.roles.find(r => r.name === sorted[parseInt(msg.content)].role))
    ...
    if (member.roles.has(role.id)) return errors.custom(message, 'That user already has that role.')
